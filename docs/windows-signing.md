...
Windows 正式发布的安装器使用 Smart Search 的**自签名代码签名证书**。新构建命名为 `SmartSearch-vX.Y.Z-windows-Setup-{x86_64,arm64}.exe`，签名状态不再放进文件名；正式发布仍要求签名和验签成功，候选的状态查看构建 `result.json`。旧 `-signed.exe` 包使用同一自签名身份，旧 `-unsigned-test.exe` 包仍未签名，请以对应版本的验签结果为准。macOS 使用 ad-hoc 签名，不具备 Apple Developer ID 或公证；Sparkle EdDSA 只验证更新包。
